...
While exploring where keyframe extraction is actually necessary, it became clear that not all videos need keyframes. If a video has minimal visual changes or the visuals don't contain meaningful information, audio-based summarization alone is usually sufficient.
...
3. **Topical Summaries**
    ...
    Uses an LLM with a structured prompt to generate summaries for each topic segment, combining both audio summaries and the associated keyframes.
...
## Tech Stack
- **Computer Vision**: OpenCV, EAST text detector, Perceptual Hashing (pHash)
...
- **General**: Python, Hugging Face, GitHub, Google Colab, Visual Studio Code
...
- Implemented the keyframe extraction process using OpenCV, EAST text detection, and perceptual hashing.
- Tested different methods and Tuned thresholds to capture meaningful frames and remove similar frames.


## Challenges & Solutions
### Challenge 1: Segmenting Topics Across Keyframes
...
**Problem**: Many keyframes looks almost identical when the visuals barely changed. Additionally, in slide-based presentations, slides often use animation functions or incremental builds, where contents reveal step by step, but only the final fully revealed slide is meaningful.

**Solution**: 
- **YOLO**: Selected keyframes when the object count or object types changed beyond a threshold.
- **OpenCV (SSIM)**: Filtered out similar frames using similarity scores and threshold.
...
After evaluating all three methods, we selected EAST + perceptual hashing as it performed the best.
